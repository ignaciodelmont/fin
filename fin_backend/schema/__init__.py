...
class Query:
    ...
    def transactions(self, root, info) -> List[Transaction]:
        ...
        transactions = transactions_resolvers.resolve_transactions(info.context["db"], user)
        print("Transactions", transactions)
        return [mp.map_res_with_type(Income if t.transaction_type() == "income" else Expense, t) for t in transactions]
...
class Mutation:
    ...
    def add_user(self, root, info, name: str, username: str, email: str) -> User:
        new_user = users_resolvers.resolve_add_user(info.context["db"], name, email, username)
        return mp.map_res_with_type(User, new_user)

    @st.mutation
    def add_income(
            self, root, info, amount: decimal.Decimal, currency: Currency, name: str, description: Optional[str] = None
    ) -> Income:
        user = info.context["user"]
